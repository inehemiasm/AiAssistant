...
class CopyToClipboardTool @Inject constructor(
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        ...
        // We do NOT scrub clipboard copies, as the user might INTENTIONALLY want to copy their own info.
...
class ShareTextTool @Inject constructor(
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        val text = args["text"] ?: return ToolResult.Error("Missing 'text' argument")
        // Sharing is a user-initiated local action, so we keep it verbatim.
...
class OpenUrlTool @Inject constructor(
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        ...
        // Scrub URLs to prevent PII leakage in query parameters if generated by the AI
        val scrubbedUrl = PiiUtils.scrub(url)
        return handleActionResult(actionExecutor.execute(OpenUrlRequest(scrubbedUrl)))
    }
}
...
class OpenMapsTool @Inject constructor(
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        ...
        // Maps queries often contain addresses, but we scrub specifically for 
        // high-risk PII like phone numbers or emails that shouldn't be in a map search.
        val scrubbedQuery = PiiUtils.scrub(query)
        return handleActionResult(actionExecutor.execute(OpenMapsRequest(scrubbedQuery)))
    }
}
...
class DraftEmailTool @Inject constructor(
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        // We do NOT scrub email drafts because they are LOCAL intents and 
        // the user expects to see the actual data in their email app.
