...
class DefaultAndroidAppActionExecutor @Inject constructor(
    ...
    private fun createCalendarEvent(request: CreateCalendarEventRequest): AppActionResult {
        val intent = Intent(Intent.ACTION_INSERT)
            ...
            .apply {
                ...
                request.location?.let { putExtra(CalendarContract.Events.EVENT_LOCATION, sanitizeInput(it)) }
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }
    ...
    private fun getAppCapabilities(appName: String): AppActionResult {
        ...
        val specificIntents = listOf(
            ...
            Intent(Intent.ACTION_VIEW, "${Constants.AppActions.HTTPS_SCHEME}example.com".toUri()) to "Opening web links (HTTP/HTTPS)",
            Intent(Intent.ACTION_VIEW, "geo:0,0".toUri()) to "Handling map/location requests",
            Intent(Intent.ACTION_SENDTO,
                (Constants.AppActions.MAILTO_SCHEME + "test@example.com").toUri()) to "Sending emails",
            Intent(Intent.ACTION_INSERT).apply { data = CalendarContract.Events.CONTENT_URI } to "Creating calendar events",
