...
class ListModelsTool @Inject constructor(
    ...
    override val description: String =
        "Returns a structured list of all AI models currently installed and available on the device."
...
class GetModelDetailsTool @Inject constructor(
    ...
    override val description: String =
        "Returns rich metadata for a specific installed model by its ID."
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        ...
        val model = registry.getInstalledModel(modelId)
            ?: return ToolResult.Error("Model '$modelId' not found.")
        val activeModelId = preferenceManager.selectedModelPreference.firstOrNull()
...
class SelectModelTool @Inject constructor(
    ...
    override val description: String =
        "Requests a switch to a different installed model. Validates model health before switching."
...
class RecommendModelTool @Inject constructor(
    ...
    override val description: String =
        "Recommends the best installed model for a specific task type (fast_chat, deep_reasoning, vision, low_storage)."
    override val inputSchema: String =
        "{\"taskType\": \"The type of task (fast_chat, deep_reasoning, vision, low_storage)\"}"
    ...
    override suspend fun execute(args: Map<String, String>): ToolResult {
        ...
        val recommendation = when (task.lowercase()) {
            "fast_chat" -> {
                val best =
                    models.filter { it.taskType == ModelTaskType.CHAT || it.taskType == ModelTaskType.VISION_CHAT }
                        .minByOrNull { it.sizeBytes ?: Long.MAX_VALUE }
                RecommendationResult(
                    best?.id,
                    best?.displayName,
                    "Smallest available chat model for quick response."
                )
            }
            ...
            "deep_reasoning" -> {
                ...
                RecommendationResult(
                    best?.id,
                    best?.displayName,
                    "Largest chat model available for complex reasoning."
                )
            }

            "vision" -> {
                val best =
                    models.find { it.capabilities.contains(ModelCapability.VISION) || it.taskType == ModelTaskType.VISION_CHAT }
                RecommendationResult(
                    best?.id,
                    best?.displayName,
                    "Best available model with vision capabilities."
                )
            }
